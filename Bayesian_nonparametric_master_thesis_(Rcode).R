...
samp.I <- function(x){ #(J,Z)
  J     <- x[1]
  Z     <- x[2]
  pr    <- PI(V.j,m,vec=TRUE)+Z*(dnorm(J,mu.j,sqrt(sig.j),log=T)+1/2*log(sig*sig.j/(sig.j+sig))) #점프를 신경써서 확률 도출
  pr    <- exp(pr-max(pr))
...
#--------------------------------------------------------------------------
n.iters       <- 32000
m             <- 20#simulation:10 realdata:20                            # cluster개수 상한
k             <- 1                             # parameter 저장
 
#결과값 저장(위의 관심 parameter 8개 & 점프 발생 횟수 B)
burn           <- n.iters/2                       #버리는 부분
gibbs.theta1   <- matrix(0,nrow=burn, ncol=9+m*3) #9개=true theta(3개)+4개(기타 para)+2개(M,B)
gibbs.theta2  <- matrix(0,nrow=burn, ncol=9+m*3) #m*3=weight(m개)+평균(m개)+분산(m개)
...
#----------------------------------------------------
  for(iter in 1:n.iters){ 
    ...
    # 각 그룹 별 점프 발생여부 판단하기 위해서
    III<-Z*II       #진짜로 점프 발생한 애들만 분류
    IIII<-vtom(III)
    G<-apply(IIII,2,sum)
    
    if(iter%%100==1) cat("- # of clusters = [",length(G[G!=0]),"]:",c(1:m)[G!=0],"\n")
    
    M<-length(G[G!=0]) #start가 1(2)일때, iter번째는 그룹이 몇개인가?
